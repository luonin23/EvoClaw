...
**持仓矩阵** (`ph-grid-nine`)：全屏响应式网格，HSL 连续热力图着色，梯度从深红(-50%)→浅红→中性暗→浅绿→深绿(+50%)，盈亏率幅度决定饱和度与亮度。顶部统计栏显示总持仓数、盈利/亏损计数、总盈亏额。
...
- 币种名去除 `USDT`/`USDC` 后缀
- **HSL 热力图着色**：`pnlToHeatBg()` 将盈亏率(-50%~+50%)映射到 HSL 色域
  - 盈利：色相 120~140（绿），饱和度/亮度随幅度递增
  - 亏损：色相 0~20（红），饱和度/亮度随幅度递增
  - 中性小盈亏：低饱和暗色，视觉收敛
- **合约数面积缩放**：格子宽度按 USDT 市值（`contracts × entry_price`）决定列数 span（1~4 列），重仓宽、轻仓窄，一眼识别仓位轻重。纯 grid 布局，不再用 `transform: scale()`，间距始终均匀。
- **排列规则**：按 `pnl_rate` 升序从左到右、从上到下排列——左上角亏损最多，右下角盈利最多。贪心换行，放不下自动折行。
- **顶部统计栏**（`#matrixHeader`）：实时显示总持仓数、盈利/亏损计数、总盈亏额，盈亏数字自动着色
- **方向指示**：格子边框颜色由方向决定——多单绿色边框、空单红色边框。边框与背景热力图叠加，方向+幅度信息互不干扰。
- **格子内容**：仅显示币名（小字，opacity 0.85）+ 盈亏率（大字，font-weight 700）。span=1 的小格子进一步缩小字号避免溢出。悬停 title 显示完整信息（币种、方向、价格、数量、盈亏、盈亏率，每行一项）。
- **毛玻璃 tooltip**：`backdrop-filter: blur(12px)`，半透明暗底 + 霓虹边框，智能防溢出定位
- 响应式适配：小屏幕逐级隐藏次要字段，字体自动缩小

**数据流**：
1. 后端 `/api/positions-map` 返回全部持仓（按 `pnl_rate` 升序排序）
2. 前端 `matrix_slots` 作为硬性显示上限：
   - 持仓 < 格子数：显示全部，剩余格子为空（虚线边框）
   - 持仓 > 格子数：显示前 N 个（亏损最多的优先），多余的隐藏

> **设计决策**：`matrix_slots`（前端显示配置）与 `max_position_count`（交易配置）完全解耦。前者只控制显示，后者只控制交易系统的开仓上限。
> 
> **设计决策**：热力图 HSL 方案优于二值红/绿着色。后者只能表达盈亏方向，HSL 梯度同时表达方向与幅度，信息密度更高。
