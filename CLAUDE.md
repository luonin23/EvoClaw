...
<!-- DESIGN_MODE_CONTEXT (auto-generated by CloudCLI) -->

You are designing with the **Stripe** brand design system.

### Visual Direction
Stripe's website is the gold standard of fintech design -- a system that manages to feel simultaneously technical and luxurious, precise and warm. The page opens on a clean white canvas (`

Apply these brand tokens consistently. Prefer the design system's colors, fonts, spacing, and shadows over generic defaults.


## Design Workflow: design-brief
...
- No gray text on colored backgrounds — use darker shade of background's own hue
- No gradient text via background-clip: text
- No glassmorphism as default
- No side-stripe borders (>1px colored left/right border on cards/items)
- No nested cards — cards inside cards is always wrong
- No identical card grids (same-sized icon+heading+text cards repeated)
- No hero-metric template (big number + small label + stats)
- No numbered section markers (01/02/03) as default scaffolding
- No tiny uppercase tracked eyebrow above every section
- No text overflowing its container at any breakpoint
- No cream/sand/beige body backgrounds (OKLCH L 0.84-0.97, hue 40-100)
- No bounce or elastic easing — use ease-out-quart/quint/expo only
- No animating <img> elements on hover (no scale/rotate on hover)
- No border-radius ≥32px on cards; cards cap at 12-16px

### Color Rules:
- Body text ≥4.5:1 contrast; large text ≥3:1; placeholder text also needs 4.5:1
- Use OKLCH for all color definitions
- Pick a color strategy: Restrained / Committed / Full palette / Drenched
- Tinted neutrals: add 0.005-0.015 chroma toward brand hue, never pure gray

### Typography Rules:
- Body line length 65-75ch max; headings use text-wrap: balance
- Pair fonts on a contrast axis (serif+sans, geometric+humanist)
- Hero display heading max: clamp() ≤6rem; letter-spacing ≥-0.04em

### Layout Rules:
- Flexbox for 1D, Grid for 2D; use repeat(auto-fit, minmax(280px, 1fr)) for responsive grids
- Semantic z-index: dropdown→sticky→modal-backdrop→modal→toast→tooltip
- Use <dialog>, popover, or portal for dropdowns — never absolute inside overflow:hidden

### Motion Rules:
- Every animation needs @media (prefers-reduced-motion: reduce)
- Reveal animations must enhance visible defaults, never gate content visibility
- Don't suppress all motion — purposeful motion is part of craft

### First-order AI-slop test:
If someone could guess the palette from the category alone, rework the design.
If someone could guess the aesthetic family from category-plus-anti-references, rework deeper.

## Output Format
Generate a complete, self-contained HTML file. Include all CSS inline in a <style> tag.
The HTML must be production-grade with working interactions, responsive design, and polished visuals.
Use the design system tokens for all colors, fonts, and spacing.
